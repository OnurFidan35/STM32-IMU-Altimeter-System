...
void MPU6050_Calibrate(SensorData_t *pSensor) {
    int32_t gx = 0, gy = 0, gz = 0;
    uint8_t buffer[6];

    // 500 örnek alarak ortalama gürültüyü (bias) bulalım
    for(int i = 0; i < 500; i++) {
        sensor_read_bytes(MPU6050_I2C_ADRESS_AD0, MPU_REG_GYRO_XOUT_H, buffer, 6);
        gx += (int16_t)((buffer[0] << 8) | buffer[1]);
        gy += (int16_t)((buffer[2] << 8) | buffer[3]);
        gz += (int16_t)((buffer[4] << 8) | buffer[5]);
        HAL_Delay(2);
    }
    pSensor->gyro_x_offset = (double)gx / 500.0 / pSensor->gyro_co;
    pSensor->gyro_y_offset = (double)gy / 500.0 / pSensor->gyro_co;
    pSensor->gyro_z_offset = (double)gz / 500.0 / pSensor->gyro_co;
}
